#!/usr/bin/env python3
"""
TIC Stats Validator - reads CSV output (serial or binary) and validates results.
...
Usage:
    ...
    python tic_stats_validator.py --port /dev/ttyUSB0 --duration 5 \
        --expected-freq-a 2000 --expected-freq-b 2000 --expected-delay 100

    # Binary mode (via CLI flag)
    python tic_stats_validator.py --binary --port /dev/tty.usbmodem1234 --duration 5 \
        --expected-freq-a 2000 --expected-freq-b 2000 --expected-delay 100

    # Binary mode (via TEST_BINARY=1 in .env)
    TEST_BINARY=1 python tic_stats_validator.py --port /dev/tty.usbmodem1234 ...
"""
...
def frame_to_ticrow(frame, seq: int) -> TicRow:
...
def read_tic_data_serial(port: str, baudrate: int, duration: float, max_lines: int,
...
def read_tic_data_binary(port: str, duration: float, max_lines: int,
                         quiet: bool = False) -> Tuple[List[TicRow], List[str], List[str]]:
    """Read TIC binary data from USB CDC port. Returns (rows, errors, non_csv_lines)."""
    from tic_binary_reader import TICReader

    rows = []
    errors = []
    non_csv_lines = []  # Not applicable for binary mode
    seq = 0
    ...
    with TICReader(port, validate=True, timeout=5.0) as reader:
        ...
        while True:
            ...
            try:
                ...
                # Check for overflow
                if frame.overflow:
                    errors.append(f"OVERFLOW in frame {frame.seq}")

                # Convert frame to TicRow
                row = frame_to_ticrow(frame, seq)
                rows.append(row)

                if not quiet:
                    print(f"  Frame{seq}: A={row.a_hz:.1f}Hz B={row.b_hz:.1f}Hz "
                          f"delay={row.d_avg_ns:.1f}ns", file=sys.stderr)
        ...
        # Check for CRC errors in validation stats
        vs = reader.validation_stats
        if vs.crc_errors > 0:
            errors.append(f"CRC errors: {vs.crc_errors}")
...
def main():
    parser = argparse.ArgumentParser(description='TIC stats validator')
    ...
    parser.add_argument('--lines', '-n', type=int, default=0,
                        help='Number of data rows to capture (default: 0 = unlimited)')
    ...
    parser.add_argument('--raw-log', type=str, default=None,
                        help='File to save raw serial output (serial mode only)')
    ...
    if binary_mode:
        print(f"Reading TIC binary data from {args.port}...", file=sys.stderr)
        rows, errors, non_csv_lines = read_tic_data_binary(
            args.port, args.duration, args.lines, args.quiet)
    else:
        print(f"Reading TIC data from {args.port}...", file=sys.stderr)
        rows, errors, non_csv_lines = read_tic_data_serial(
            args.port, args.baudrate, args.duration, args.lines, args.quiet, args.raw_log)
